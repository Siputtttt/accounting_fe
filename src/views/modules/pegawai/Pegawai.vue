...
const fetchData = async (event) => { 
    ...
    try {
        const response = await store.getData({
            url     :  `api/pegawai?s=${textSearch.value}`  
        })  
        if (response.data.status === 1) {     
            let onColumns = []
            response.data.columns.map( item => {
                if( item.format_as =='image') {
                   // item.formatter = formatImage
                }
                onColumns.push( item )
            })
            console.log(onColumns )
            columns.value = onColumns
...
const edit = async (item) => { 
    if(is_edit.value == 1) { 
        loading.value = true ;
        const response = await store.getData({
            url     :  `api/pegawai/edit?id=${item.id}`  
        })  
        if (response.data.status === 1) {   
            posts.value = response.data.data 
            modal.value = true 
...
const doDownload = async (event) => {
    ...
    try {
        const response = await store.getData({
            url     :  `api/pegawai/download?dw=${download.value.startRows}&e=${download.value.endRows}&t=${download.value.type}&s=${textSearch.value}`  
        })  
        proxy.$export(  response.data ,  'pegawai.xls');
        proxy.$messageSuccess(  proxy ,  'Success' ,response.data.message )        
...
const doPrint = async (event) => {  
    proxy.$printArea( 'printedArea','Daftar Pegawai')    
}
const remove  = async (event) => {
    ...
    if (result.isConfirmed) {
        const response = await store.deleteData({
            url: "api/pegawai/" + selected.value ,
            headers: {
                "Content-Type": "application/json",
            },
        responseType: "json",
...
<template>
  <div class="wrapper">
    <div class="header-nav">
      <h3><i class="bi bi-layout-sidebar"></i> Daftar Pegawai </h3>
    </div>
    <div class="page"> 
        <div class="toolbar-nav p-2 mb-3 ">
            <div class="row">
                <div class="col-md-6  ">
                    <button v-if="access.is_add =='1'" class="btn btn-sm btn-light  mr-2" @click="add"><i class="bi bi-plus"></i> New </button>
                    <button  v-if="access.is_delete =='1'" class="btn btn-sm btn-light mr-2 " @click="remove"><i class="bi bi-trash3 text-danger"></i> Delete </button>                    
                    <button  v-if="access.is_print =='1'" class="btn btn-sm btn-light  mr-2" @click="doPrint"><i class="bi bi-printer text-success"></i> Print </button>
                    <button  v-if="access.is_excel =='1'" class="btn btn-sm btn-light mr-2 " @click="downloadModal = true"><i class="bi bi-file-earmark-excel"></i>  Download </button> 

                    <button   class="btn btn-sm btn-light  mr-2" @click="searchModal = true" ><i class="bi bi-search"></i> More Search .. </button>
                    <button class="btn btn-sm btn-light  mr-2"  @click="fetchData"><i class="bi bi-arrow-clockwise text-info"></i> Reload </button> 
                </div>
                <div class="col-md-3"> 
    ...
    <BModal v-model="modal" title="Form Daftar Pegawai"  size="md" no-footer>
    <div style="min-height: 350px;">
            <BForm   @submit.prevent="onSubmit">
                ...
                <button type="submit" class="btn btn-sm btn-success"  style="font-size: 11px;"  > Save Change(s)</button>
            </BForm>
