...
<template>
    <div class="toolbar-nav border-0">
        <div class="row justify-content-between align-items-center gap-2">
            <div class="col-md-6 d-flex align-items-center gap-3">
                ...
                <button type="button" class="btn btn-sm btn-light border" @click="modalPrint = true"><i class="bi bi-printer"></i> Print</button>
            </div>
            <div class="col-md-3">
        ...
        <BForm @submit.prevent="saveItem">
            <BFormGroup label="Item" label-for="no" class="mb-3">
                <v-select :options="itemRented" v-model="form.id_items_rented" label="name" :reduce="(option) => option.id" placeholder="Pilih Item" size="sm">
                    <template #option="{ name, units, units_label, price }"> {{ name }} - {{ units }} {{ units_label }} (Rp {{ price }}) </template>
                </v-select>
            </BFormGroup>
    ...
    <BModal v-model="modalPrint" title="Preview Invoice" size="xl" no-header no-footer no-close-on-backdrop scrollable>
        <InvoicePrint :invoice="props.invoice" :items="item" />
        <div class="text-right d-flex justify-content-end gap-2 mt-5">
