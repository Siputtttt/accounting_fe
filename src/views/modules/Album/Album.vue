...
const fetchData = async (event) => {
  ...
  try {
    const response = await store.getData({
      url: `api/Album?s=${textSearch.value}`,
    });
...
const doDownload = async (event) => {
  ...
  try {
    const response = await store.getData({
      url: `api/Album/download?dw=${download.value.startRows}&e=${download.value.endRows}&t=${download.value.type}&s=${textSearch.value}`,
    });
    proxy.$export(response.data, "Album.xls");
    proxy.$messageSuccess(proxy, "Success", response.data.message);
...
const doPrint = async (event) => {
  proxy.$printArea("printedArea", "Album");
};
...
<template>
  <div class="wrapper">
    <div class="header-nav">
      <h3><i class="bi bi-layout-sidebar"></i> Album</h3>
    </div>
    <div class="toolbar-nav mb-3">
      <div class="row">
        <div class="col-md-6">
          <div v-if="access.is_add == '1'" class="mr-2 tool-icon" @click="add">
            <i class="bi bi-file-earmark-plus"></i>
          </div>
          <div v-if="access.is_delete == '1'" class="mr-2 tool-icon" @click="remove">
            <i class="bi bi-trash3"></i>
          </div>
          <div v-if="access.is_print == '1'" class="mr-2 tool-icon" @click="doPrint">
            <i class="bi bi-printer"></i>
          </div>
          <div
            v-if="access.is_excel == '1'"
            class="mr-2 tool-icon"
            @click="downloadModal = true"
          >
            <i class="bi bi-file-earmark-excel"></i>
          </div>
          <div class="tool-icon mr-2" @click="searchModal = true">
            <i class="bi bi-search"></i>
          </div>
          <div class="tool-icon mr-2" @click="fetchData">
            <i class="bi bi-arrow-clockwise"></i>
          </div>
        </div>
        <div class="col-md-3">
          <BFormCheckbox
            v-if="access.is_edit == '1'"
            v-model="is_edit"
            value="1"
            unchecked-value="0"
            >Enable Edit Mode
      ...
      >
        <div class="row">
          <template v-for="(row, index) in rows" :key="index">
            <div class="col-md-3" @click="edit(row)">
              <div class="card gallery">
                {{ row.name }}

                <div v-html="row.thumbnail" class="img-wrapper"></div>
              </div>
            </div>
          </template>
        </div>
