...
const fetchData = async (event) => { 
    ...
    try {
        const response = await store.getData({
            url     :  `api/unitkerja?s=${textSearch.value}`  
        })  
...
const edit = async (item) => { 
    if(is_edit.value == 1) { 
        loading.value = true ;
        const response = await store.getData({
            url     :  `api/unitkerja/edit?id=${item.id}`  
        })  
        if (response.data.status === 1) {   
            posts.value = response.data.data 
            modal.value = true 
...
const doDownload = async (event) => {
    ...
    try {
        const response = await store.getData({
            url     :  `api/unitkerja/download?dw=${download.value.startRows}&e=${download.value.endRows}&t=${download.value.type}&s=${textSearch.value}`  
        })  
        proxy.$export(  response.data ,  'unitkerja.xls');
        proxy.$messageSuccess(  proxy ,  'Success' ,response.data.message )        
...
const doPrint = async (event) => {  
    proxy.$printArea( 'printedArea','Unit Kerja')    
}
const remove  = async (event) => {
    ...
    if (result.isConfirmed) {
        const response = await store.deleteData({
            url: "api/unitkerja/" + selected.value ,
            headers: {
                "Content-Type": "application/json",
            },
        responseType: "json",
...
<template>
  <div class="wrapper">
        ...
        <h3><i class="bi bi-layout-sidebar"></i> Unit Kerja </h3>
        </div>
    ...
    <BModal v-model="modal" title="Form Unit Kerja"  size="md" no-footer>
                ...
                <div class="row">
                    <template v-for="form in forms">
                        ...
                        <template v-if="form.type =='files'" >
                            <BFormGroup   :label="form.label" class="mb-3 " :class="'col-md-'+form.size" >  
                                <BFormFile  v-model="posts[form.key]"  size="sm" /> 
                            </BFormGroup>
                        </template>
