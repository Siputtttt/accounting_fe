...
const fetchData = async (event) => { 
    ...
    try {
        const response = await store.getData({
            url     :  `api/eselon?s=${textSearch.value}`  
        })  
...
const edit = async (item) => { 
    if(is_edit.value == 1) { 
        loading.value = true ;
        const response = await store.getData({
            url     :  `api/eselon/edit?id=${item.id}`  
        })  
        if (response.data.status === 1) {   
            posts.value = response.data.data 
            modal.value = true 
...
const doDownload = async (event) => {
    ...
    try {
        const response = await store.getData({
            url     :  `api/eselon/download?dw=${download.value.startRows}&e=${download.value.endRows}&t=${download.value.type}&s=${textSearch.value}`  
        })  
        proxy.$export(  response.data ,  'eselon.xls');
        proxy.$messageSuccess(  proxy ,  'Success' ,response.data.message )        
...
const doPrint = async (event) => {  
    proxy.$printArea( 'printedArea','Eselonering')    
}
...
<template>
  <div class="wrapper">
    <div class="header-nav">
      <h3><i class="bi bi-layout-sidebar"></i> Eselonering </h3>
    </div>
    ...
    <BModal v-model="modal" title="Form Eselonering"  size="md" no-footer :no-close-on-backdrop="true">
    <div  >
